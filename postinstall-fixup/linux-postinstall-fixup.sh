...
for deps_file in postinstall_deps_SOFA.tmp postinstall_deps_plugin_*.tmp; do
    ...
    groups="libpng libicu libmng libxcb libxkb libpcre2 libjbig libwebp libjpeg libsnappy libtiff libglfw libnfd"
...
if [ -x "$(command -v patchelf)" ]; then
    defaultRPATH='$ORIGIN/../lib:$ORIGIN/../../../bin/python/lib'
    (
    find "$INSTALL_DIR" -type f -name "*.so.*"
    find "$INSTALL_DIR" -type f -name "runSofa" -path "*/bin/*"
    ) | while read lib; do
        patchelf --set-rpath $defaultRPATH:"$(patchelf --print-rpath $lib)" $lib
