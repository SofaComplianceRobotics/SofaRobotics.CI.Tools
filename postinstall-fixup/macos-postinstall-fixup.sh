...
check-all-deps() {
    ...
    find "$INSTALL_DIR" -type f -name "runSofa*" -path "*/bin/*"
        ...
        if echo "$dependencies" | grep --quiet "/libpython" ||
           ...
           echo "$dependencies" | grep --quiet "/libglfw"  ||
           echo "$dependencies" | grep --quiet "/libnfd"  ||
        ...
        (echo "$dependencies") | while read dep; do
            if libpython="$(echo $dep | egrep -o "/libpython[^ ]*\.dylib"  | cut -c2-)" && [ -n "$libpython" ]; then
                ...
                libname="$libglew"
            elif libglfw="$(echo $dep | egrep -o "/libglfw[^\/]*?\.dylib$" | cut -c2-)" && [ -n "$libglfw" ]; then
                libname="$libglfw"
            elif libnfd="$(echo $dep | egrep -o "/libnfd[^\/]*?\.dylib$" | cut -c2-)" && [ -n "$libnfd" ]; then
                libname="$libnfd"
...
if [ -d "$BUNDLE_DIR" ]; then
    ...
    find "$BUNDLE_DIR" -type f -name "runSofa*" -path "*/bin/*"
        ...
        install_name_tool -add_rpath "@loader_path/../../../bin/python/lib" $lib 2>> install_name_tool.errors.log
